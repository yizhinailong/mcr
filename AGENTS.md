...
- `src/options/`: request transfer configuration in namespace `mcr::options`. `transfer_options.cppm` exports common timing, rate, connection selection, capacity, and diagnostic options through `mcr.transfer_options`. Keep related small option types together; authentication, proxies, redirects, protocol selection, and TLS options retain their dedicated modules.
