...
def discover_examples():
    ...
    for name, target in manifest["targets"].items():
        ...
        examples.append(Example(name, api, method, batch, download))
    require(covered == declarations, f"API coverage differs: missing {declarations - covered}; extra {covered - declarations}")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_request(self):
        ...
        if request_id == "first":
            time.sleep(0.05)
        status = 200 if url.path in {"/echo", "/binary"} else 404
        if status == 404:
            output = b"not found"
        ...
        self.send_header("Content-Type", "application/octet-stream" if url.path == "/binary" else "text/plain")
...
def verify_http(example, binary, fixture, environment, output_path, status):
    ...
    endpoint = "/status/404" if status == 404 else "/binary" if example.download else "/echo"
...
def main():
    ...
    if args.serve:
        with Fixture(args.port) as fixture:
            print(f"Serving http://127.0.0.1:{fixture.server_port}/echo and /binary (Ctrl+C to stop)", flush=True)
            try:
