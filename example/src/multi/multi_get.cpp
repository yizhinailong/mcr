/**
 * @file multi_get.cpp
 * @brief Demonstrate mcr::MultiGet with an explicit call to this public request API.
 */
import std;
import mcr;
...
auto main(int argc, char** argv) -> int {
    return example::run(argc, argv, "multi_get", [](example::Arguments const& args) {
        ...
        auto second{ std::tuple{
            ...
            mcr::UserAgent{ "mcr_example" },
            mcr::options::Timeout{ std::chrono::seconds{ 10 } }} };
        auto responses{ mcr::MultiGet(std::move(first), std::move(second)) };
