/**
 * @file put_coro.cpp
 * @brief Demonstrate mcr::PutCoro with an explicit call to this public request API.
 */
import std;
import mcr;
...
/**
 * @brief Await a PUT request while owning the URL in the coroutine frame.
...
auto request(std::string url) -> mcr::Task<mcr::Result<mcr::Response>> {
    co_return co_await mcr::PutCoro(
        mcr::Url{
            std::move(url)
    },
...
auto main(int argc, char** argv) -> int {
    return example::run(argc, argv, "put_coro", [](example::Arguments const& args) { return example::print_response(mcr::sync_wait(request(args.url))); }, example::RuntimeKind::Coroutine);
}
