/**
 * @file head_coro.cpp
 * @brief Demonstrate mcr::HeadCoro with an explicit call to this public request API.
 */
import std;
import mcr;
...
/**
 * @brief Await a HEAD request while owning the URL in the coroutine frame.
...
auto request(std::string url) -> mcr::Task<mcr::Result<mcr::Response>> {
    co_return co_await mcr::HeadCoro(
        mcr::Url{
            std::move(url)
    },
...
auto main(int argc, char** argv) -> int {
    return example::run(argc, argv, "head_coro", [](example::Arguments const& args) { return example::print_response(mcr::sync_wait(request(args.url))); }, example::RuntimeKind::Coroutine);
}
