/**
 * @file post_coro.cpp
 * @brief Demonstrate mcr::PostCoro with an explicit call to this public request API.
 */
import std;
import mcr;
...
/**
 * @brief Await a POST request while owning the URL in the coroutine frame.
...
auto main(int argc, char** argv) -> int {
    return example::run(argc, argv, "post_coro", [](example::Arguments const& args) { return example::print_response(mcr::sync_wait(request(args.url))); }, example::RuntimeKind::Coroutine);
}
