/**
 * @file delete_coro.cpp
 * @brief Demonstrate mcr::DeleteCoro with an explicit call to this public request API.
 */
import std;
import mcr;
...
/**
 * @brief Await a DELETE request while owning the URL in the coroutine frame.
...
auto request(std::string url) -> mcr::Task<mcr::Response> {
    co_return co_await mcr::DeleteCoro(
        mcr::Url{
            std::move(url)
    },
...
auto main(int argc, char** argv) -> int {
    return example::run(argc, argv, "delete_coro", [](example::Arguments const& args) { return example::print_response(mcr::sync_wait(request(args.url))); }, example::RuntimeKind::Coroutine);
}
