/**
 * @file multi_patch_async.cpp
 * @brief Demonstrate mcr::MultiPatchAsync with an explicit call to this public request API.
 */
import std;
import mcr;
...
auto main(int argc, char** argv) -> int {
    return example::run(argc, argv, "multi_patch_async", [](example::Arguments const& args) {
        ...
        auto second{ std::tuple{
            ...
            mcr::UserAgent{ "mcr_example" },
            mcr::options::Timeout{ std::chrono::seconds{ 10 } }} };
        auto pending{ mcr::MultiPatchAsync(std::move(first), std::move(second)) };
