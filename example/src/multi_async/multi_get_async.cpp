/**
 * @file multi_get_async.cpp
 * @brief Demonstrate mcr::MultiGetAsync with an explicit call to this public request API.
 */
import std;
import mcr;
...
auto main(int argc, char** argv) -> int {
    return example::run(argc, argv, "multi_get_async", [](example::Arguments const& args) {
        ...
        auto second{ std::tuple{
            ...
            mcr::UserAgent{ "mcr_example" },
            mcr::options::Timeout{ std::chrono::seconds{ 10 } }} };
        auto pending{ mcr::MultiGetAsync(std::move(first), std::move(second)) };
